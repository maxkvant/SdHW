...
`GameController` сначала находится в состоянии генерации модели (`generation of model`). После того как модель сгенерируется переходит в состояние `Gaming` -- игровой процесс. Игровой процесс прерывается по нажатию `q`. Или после нажатия `r` или после смерти игрока модель генерируется заново (возвращаемся в состояние `generation of model`).  
